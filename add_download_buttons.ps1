# PowerShell script to add download buttons to all HTML files in the graphs directory
...
Write-Host "Found $($htmlFiles.Count) HTML files to update (excluding case1 and case2 which are already updated)"
...
foreach ($file in $htmlFiles) {
    ...
    if ($htmlContent -match "download-buttons") {
        Write-Host "  Skipping $($file.Name) - download buttons already exist"
        continue
    }
    
    # Add CSS styles before the closing style tag
    $htmlContent = $htmlContent -replace '(\s+)</style>', "$downloadCss`n$1</style>"
    
    # Add download buttons after the last filter label
    $htmlContent = $htmlContent -replace '(\s+)</label>(\s+)</div>', '$1</label>$2$downloadButtonsHtml$1</div>'
    
    # Add JavaScript functions before the closing script tag
    $htmlContent = $htmlContent -replace '(\s+)</script>', "$downloadJs`n$1</script>"
    
    # Write the updated file
    Set-Content -Path $file.FullName -Value $htmlContent -Encoding UTF8
